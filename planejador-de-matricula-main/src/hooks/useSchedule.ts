...
import { Course, CourseStatus, DayOfWeek, DragSelection, RawScheduleJSON } from '@/types/schedule';
...
export function useSchedule(initialData?: {
  courses: Course[];
  confirmedIds: string[];
  plannedIds: string[];
}) {
  const [courses, setCourses] = useState<Course[]>(initialData?.courses || mockCourses);
  const [plannedIds, setPlannedIds] = useState<Set<string>>(new Set(initialData?.plannedIds || []));
  ...
  const filteredCourses = useMemo(() => {
    return courses.filter(course => {
      ...
      if (filters.professors.length > 0 && !filters.professors.includes(course.professor)) return false;
      if (filters.degrees.length > 0 && (!course.degree || !filters.degrees.includes(course.degree))) return false;
      if (filters.periods.length > 0 && !filters.periods.includes(course.period)) return false;
  ...
  const setData = useCallback((newData: { courses: Course[]; confirmedIds: string[]; plannedIds: string[], completedCodes?: string[] }) => {
    ...
    setPlannedIds(new Set(newData.plannedIds));
