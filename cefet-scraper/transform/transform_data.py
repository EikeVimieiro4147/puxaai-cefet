...
CURRICULO_INTEGRALIZACAO_PATH = "data/curriculo_integralizacao.json"
...
def run_transformation():
    ...
    """
    if os.path.exists(CURRICULO_INTEGRALIZACAO_PATH):
        with open (CURRICULO_INTEGRALIZACAO_PATH, "r", encoding="utf-8") as f:
            curriculo_data = json.load(f)
            curriculo_disciplinas = curriculo_data.get("disciplinas", [])
    else:
        curriculo_data = {}
        curriculo_disciplinas = []
        
    disciplinas_aprovadas = []
    for item in curriculo_disciplinas:
        if item.get("situacao") == "Vencido":
            disciplinas_aprovadas.append(item.get("disciplina"))
